...
const FALLBACK_TESTIMONIALS = [
  {
    ...
    image: '/images/paul_jeggels_customer_1_sm.webp',
    rating: 5,
  },
  {
    ...
    name: 'Sarah T.',
    location: 'Cape Town',
    image: '/images/paul_jeggels_customer_2_sm.webp',
    rating: 5,
  },
  {
    ...
    image: '/images/paul_jeggels_customer_3_sm.webp',
    rating: 5,
  },
];
...
const StarRating = ({ rating }) => (
  <div className="flex gap-1 mb-4">
    {[...Array(5)].map((_, j) => (
      <Star
        key={j}
        className={`w-4 h-4 ${j < rating ? 'fill-pjd-terra text-pjd-terra' : 'text-pjd-cream/30'}`}
      />
    ))}
  </div>
);
...
const Testimonials = () => {
  ...
  return (
    <section className="bg-pjd-dark py-20">
      <div className="max-w-7xl mx-auto px-6">
        <p className="text-pjd-cream/60 text-xs font-bold tracking-[0.25em] uppercase mb-3 text-center font-body">
          {isGoogle ? 'Google Reviews' : 'What Surfers Say'}
        </p>
        <h2 className="text-3xl md:text-4xl font-bold text-pjd-cream text-center mb-4 font-display">
          ...
          <div className="flex items-center justify-center gap-2 mb-10">
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, j) => (
                <Star
                  key={j}
                  className={`w-4 h-4 ${j < Math.round(meta.rating) ? 'fill-pjd-terra text-pjd-terra' : 'text-pjd-cream/30'}`}
                />
              ))}
            </div>
            <span className="text-pjd-cream/70 text-sm font-body">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map(({ quote, name, location, image, rating }, i) => (
            <div
              key={i}
              className="bg-white/10 backdrop-blur-sm border border-white/15 p-8 flex flex-col"
            >
              <StarRating rating={rating || 5} />
              <p className="text-pjd-cream/80 leading-relaxed flex-1 italic mb-6 font-body">"{quote}"</p>
              <div className="flex items-center gap-3 pt-4 border-t border-pjd-cream/10">
                {image && (
                  <img
                    src={image}
                    alt={name}
                ...
                <div>
                  <p className="text-pjd-cream font-bold text-sm font-body">{name}</p>
                  <p className="text-pjd-cream/70 text-xs font-body">{location}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-4 mt-10">
            <button
              ...
              disabled={page === 0}
              className="p-2 rounded-full border border-pjd-cream/20 hover:bg-pjd-cream/10 transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
              aria-label="Previous reviews"
            >
              <ChevronLeft className="w-5 h-5 text-pjd-cream" />
            </button>
            <span className="text-pjd-cream/60 text-sm font-body">
              {page + 1} / {totalPages}
            </span>
            <button
              onClick={() => setPage((p) => p + 1)}
            ...
            >
              <ChevronRight className="w-5 h-5 text-pjd-cream" />
            </button>
          </div>
        )}
      </div>
